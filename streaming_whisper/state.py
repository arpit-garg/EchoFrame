...
class State:
    ...
    def _extract_transcriptions(
        ...
        for word in ts_result.words:
            ...
            if word.end < last_pause.end:
                ...
                logging.debug(f'Participant {self.participant_id}: final is "{final}"')
            # consider everything else as interim
                ...
                logging.debug(f'Participant {self.participant_id}: interim is "{interim}"')

        if final.strip():
            cut_mark_bytes = self.get_num_bytes_for_slicing(last_pause.end)
            if cut_mark_bytes > 0:
                logging.debug(f'Participant {self.participant_id}: cut mark set at {cut_mark_bytes} bytes')
    ...
    async def process(self, chunk: Chunk, previous_tokens: list[int]) -> List[utils.TranscriptionResponse] | None:
        ...
        if not self.long_silence and not self.is_transcribing:
            ...
            if len(results) > 0:
                return results
        logging.debug(f'Participant {self.participant_id}: no ts results')
        return None
    ...
    async def add_to_store(self, chunk: Chunk, tmp_working_audio: bytes = b''):
        ...
        logging.debug(f'## Participant {self.participant_id}: speech timestamps {speech_timestamps}')
        logging.debug(f'## Participant {self.participant_id}: last speech timestamp {self.last_speech_timestamp}')
        ...
        if speech_timestamps and speech_timestamps[-1]['end'] != self.last_speech_timestamp:
            ...
            logging.debug(f'## Participant {self.participant_id}: chunk is silent')
            ...
            if speech_timestamps and audio_length_seconds - speech_timestamps[-1]['end'] >= 1:
                logging.debug(f'## Participant {self.participant_id}: long silence detected')
    ...
    def reset(self):
        ...
        """
        logging.debug(f'Participant {self.participant_id}: flushing working audio')
        self.working_audio_starts_at = 0
        self.working_audio = b''
        self.last_speech_timestamp = 0.0
    ...
    def get_num_bytes_for_slicing(cut_mark: float) -> int:
        ...
        logging.debug(f'Sliceable bytes: {sliceable_bytes}')
    ...
    async def do_transcription(self, audio: bytes, previous_tokens: list[int]) -> utils.WhisperResult | None:
        ...
        logging.debug(f'Participant {self.participant_id}: starting transcription of {len(audio)} bytes.')
        try:
            ts_result = await loop.run_in_executor(None, utils.transcribe, [audio], self.lang, previous_tokens)
        except RuntimeError as e:
            logging.error(f'Participant {self.participant_id}: failed to transcribe {e}')
            self.is_transcribing = False
            return None
        end = time.perf_counter_ns()
        processing_time = (end - start) / 1e6 / 1000
        logging.debug(ts_result)
        self.is_transcribing = False
        return ts_result
